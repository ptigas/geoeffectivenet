...
def basis_matrix(nmax, theta, phi):
    ...
    return  basis.reshape(-1, theta.shape[0], theta.shape[1]).swapaxes(0, 1).swapaxes(2, 1)
...
class ShpericalHarmonicsDataset(data.Dataset):
    def __init__(self,
        ...
        # add dipole
        self.omni = np.concatenate([self.omni, dipole_tilt(self.dates).reshape(-1, 1)], 1)
        ...
        if scaler is not None:
            ...
            self.supermag_data[self.target_idx] = scaler['supermag'].transform(self.supermag_data[self.target_idx])
        else:
            self.scaler = {}
            ...
            self.supermag_data[self.target_idx] = self.scaler['supermag'].fit_transform(self.supermag_data[self.target_idx])
    ...
    def __getitem__(self, index):
        ...
        if self.future_length > 1:
            future_supermag = torch.Tensor(self.supermag_data[index+self.lag:index+self.future_length+self.lag])
            future_supermag = torch.nn.AvgPool1d(self.future_length)(future_supermag.permute([1, 2, 0]))[..., 0].unsqueeze(0).numpy()
        else:
            future_supermag = self.supermag_data[index+self.lag:index+self.future_length+self.lag]

        sm_future = NamedAccess(
              future_supermag,
              self.supermag_features)
        mlt = sm_future['MLT']
        maglat = sm_future['MAGLAT']

        _mlt = 90.0-sm_future['MLT']/24.0*360.0
        _mcolat = 90.0-sm_future['MAGLAT']

        return self.omni[index-self.past_omni_length:index], \
