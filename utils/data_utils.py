...
def get_iaga_data(path, tiny=False):
    ...
    if tiny:
        files = sorted([f for f in glob.glob(path + 'supermag_iaga_tiny*.npz')], key=lambda f: int(re.sub('\D', '', f)))
    else:
        files = sorted([f for f in glob.glob(path + 'supermag_iaga_[!tiny]*.npz')], key=lambda f: int(re.sub('\D', '', f)))
    assert len(files) > 0
    ...
    for i, d in enumerate(data):
        data[i] = np.concatenate([
                    d,
                    np.zeros([d.shape[0], max_stations-d.shape[1], d.shape[2]])*np.nan],
                    axis=1)
