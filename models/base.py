...
class BaseModel(pl.LightningModule):
    ...
    def training_step(self, training_batch, batch_idx):
        past_omni, past_supermag, future_supermag, past_dates, future_dates, (phi, theta) = training_batch
        ...
        self.log('train_r2', R2(future_supermag, predictions).mean(), on_step=False, on_epoch=True)
        return loss

    def validation_step(self, val_batch, batch_idx):
        past_omni, past_supermag, future_supermag, past_dates, future_dates, (mlt, mcolat) = val_batch
        ...
        self.log('val_R2', R2(future_supermag, predictions).mean(), on_step=False, on_epoch=True)
        ...
        if batch_idx == 0:
            ...
            coeffs = []
            targets = []
            for past_omni, past_supermag, future_supermag, past_dates, future_dates, (mlt, mcolat) in self.wiemer_data:
                ...
                _, _coeffs, pred = self(past_omni, past_supermag, mlt, mcolat, past_dates, future_dates)
            ...
            self.log('wiemer_R2', R2(targets, predictions).mean(), on_step=False, on_epoch=True)
            self.log('wiemer_MSE', ((targets - predictions) ** 2).mean(), on_step=False, on_epoch=True)

            fig, ax = plt.subplots()

            ax.scatter(targets.cpu().numpy().ravel(), predictions.cpu().numpy().ravel())
            self.logger.experiment.log({"wiemer_scatter": [wandb.Image(get_img_from_fig(fig), caption="val_scatter")]})
            nice_idx = [500]
            pred_sphere = spherical_plot_forecasting(self.nmax, coeffs[nice_idx], predictions[nice_idx], targets[nice_idx])
            self.logger.experiment.log({"pred_sphere": [wandb.Image(pred_sphere, caption="pred_sphere")]})
